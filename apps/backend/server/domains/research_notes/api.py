...
from server.application.web_support import effective_user_profile, login_required_page, page_context, project_repository, research_note_repository, signature_repository
...
@require_GET
def research_note_files_api(request, note_id: str):
    note_obj = ResearchNote.objects.select_related("project").filter(id=note_id).first()
    if not note_obj:
        raise Http404("Research note not found")

    if note_obj.project_id:
        profile = effective_user_profile(request) or {}
        if not project_repository.can_view_project(str(note_obj.project_id), profile):
            return JsonResponse({"detail": "권한이 없습니다."}, status=403)

    return JsonResponse(research_note_repository.list_note_files(note_id), safe=False)
