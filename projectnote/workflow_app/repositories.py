...
from .models import DataUpdate, Project, ProjectMember, ResearchNote, ResearchNoteFile, ResearchNoteFolder, Researcher, SignatureState


class WorkflowRepository:
    def list_projects(self) -> list[dict]:
        return [self._project_to_dict(project) for project in Project.objects.order_by("-created_at")]
    ...
    def create_project_members(self, project: Project, invited: list[InvitedMemberCommand]) -> None:
        researchers = {researcher.id: researcher for researcher in Researcher.objects.filter(id__in=[i.researcher_id for i in invited])}
        for item in invited:
    ...
    def list_research_notes(self) -> list[dict]:
        return [self._note_to_dict(note) for note in ResearchNote.objects.order_by("-updated_at")]
    ...
    def update_signature(self, signed_by: str, status: str) -> dict:
        ...
        return self._signature_to_dict(signature)

    def read_signature(self) -> dict:
        return self._signature_to_dict(self.get_or_create_signature_state())
    ...
    @staticmethod
    def _note_to_dict(note: ResearchNote) -> dict:
        return {
            "id": str(note.id),
            "title": note.title,
            "owner": note.owner,
    ...
    @staticmethod
    def _signature_to_dict(signature: SignatureState) -> dict:
        return {
